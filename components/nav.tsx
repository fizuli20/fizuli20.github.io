...
export function Nav() {
  ...
  return (
    ...
    >
      <nav
        aria-label="Primary"
        className="mx-auto flex h-[58px] max-w-[1120px] items-center justify-between gap-3 px-4 md:px-6 lg:grid lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] lg:px-8 xl:px-10"
      >
        {/* Left — personal logo + wordmark */}
        <a
          href="#top"
          aria-label="Fizuli Hasanov — Home"
          className="group flex min-w-0 items-center gap-2 rounded-full border border-transparent py-1 pr-2 transition-all duration-500 hover:border-white/[0.08] hover:bg-white/[0.025] lg:justify-self-start"
        >
          <span className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white/[0.045] shadow-[inset_0_1px_0_rgba(255,255,255,0.13),0_14px_38px_rgba(0,0,0,0.35)] ring-1 ring-white/10 transition-all duration-500 group-hover:bg-white/[0.085] group-hover:ring-white/25 group-hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_16px_44px_rgba(0,0,0,0.4),0_0_28px_rgba(255,255,255,0.08)]">
            <span
              aria-hidden
          ...
          <span className="hidden min-w-0 max-w-[210px] flex-col leading-none sm:flex">
            <span className="truncate font-mono text-[11px] font-semibold tracking-[0.16em] text-white transition-colors duration-500">
              FIZULI HASANOV
            </span>
            <span className="mt-1.5 flex items-center gap-1.5 truncate font-mono text-[8px] tracking-[0.2em] text-zinc-500 transition-colors duration-500 group-hover:text-zinc-300">
              <span
                aria-hidden
        ...
        <ul className="glass-surface hidden items-center gap-0.5 rounded-full border border-white/10 bg-white/[0.035] p-1 backdrop-blur-2xl lg:flex lg:justify-self-center">
          {links.map((l) => (
            <li key={l.id} className="relative">
              <a
                href={`#${l.id}`}
                className={`relative z-10 block rounded-full px-3 py-1.5 font-mono text-[11px] transition-colors duration-300 ${
        ...
        <div className="flex min-w-0 items-center gap-3 lg:justify-self-end">
